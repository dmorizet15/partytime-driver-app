...
| Item | Status |
|---|---|
| Active feature | **TapGoods Item Check-Off (driver UI)** — hard completion gate + TapGoods write-back on delivery/pickup stops (driver `ec2fe6e`, pushed to `main` 2026-06-10, build green). Dashboard side pre-shipped (`87c75f2`, mig 096 applied + live-verified). **Darren's live test is the completion gate — NOT yet smoke-tested.** Prior: warehouse IN TRANSIT writer (driver `eaedfb2` + dashboard `ff5c8eb`, 2026-06-09) |
| Latest merge | `ec2fe6e` (driver): TapGoods item check-off — `ItemCheckoffSheet` hard gate in `handleMarkCompleteTap`, `stop_item_checkoffs` audit inserts, write-back POST to dashboard `/api/tapgoods/dispatch/write-back`, offline queue, damage→WO plumbing. Prior: warehouse IN TRANSIT writer (`eaedfb2`/`ff5c8eb`) |
...
Shipped to `main` 2026-06-10 (`ec2fe6e`), build green. **NOT smoke-tested — Darren's live test (real delivery/pickup → TapGoods reflects quantities + discrepancy email lands + WO fires) is the completion gate.** Spec (locked, merge gate): Notion `37b0aa6451b881e39a1bcde70e6bd288`. Dashboard side pre-shipped (`87c75f2`): mig 096 (`stop_item_checkoffs` + `dispatch_stops.tapgoods_discrepancy_emailed_at`, applied + live-verified) + `POST /api/tapgoods/dispatch/write-back` + `sendStopDiscrepancyEmail`.
