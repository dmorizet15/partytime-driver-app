...
- **Types**: `tapgoods_pick_list_item_id` added to `RawItem` (supabaseTransform) + `Stop['items']` — types-only, the value always flowed at runtime. `src/types/supabase.ts` was **hand-patched** from the dashboard's post-096 regen (CLI had no access token this session) — run a real `supabase gen types` regen when logged in.
