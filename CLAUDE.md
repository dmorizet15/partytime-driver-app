...
| Latest merge | `3f7d01a` (driver): Will Call Phase 1 surface (`src/lib/willCall/`, `src/hooks/willCall/`, `src/screens/willCall/`, `src/components/willCall/`, `/api/will-call`, BottomNav + ToolsScreen swap). `cb9453f` (dashboard): `requireWillCallAccess` dual auth + role gate + CORS on the four Will Call action routes. Prior: `8869246`/`8c8fbcf` check-off revisions |
...
- **Head tags via the Next 14 Metadata API, NOT raw `<head>`** (`src/app/layout.tsx`). In the App Router you do not hand-write `<head>` in the root layout — `metadata`/`viewport` exports emit identical tags and avoid hydration warnings. Added: `metadata.manifest`, `metadata.appleWebApp {capable, statusBarStyle:'black-translucent', title:'PTR Work'}`, `metadata.icons {icon:favicon, apple:apple-touch-icon}`, `viewport.themeColor:'#1F46FF'`. Existing `title`/`description`/viewport fields were extended, not duplicated.
