...
| Item | Status |
|---|---|
| Active feature | **TapGoods Item Check-Off — live-test revisions 1–3** (driver `8869246` + dashboard `8c8fbcf`, pushed to `main` both repos 2026-06-10, both builds green): Rev 1 inline check-off + single gated CTA (sheet retired), Rev 2 co-driver permission fixes (inspection lock + transfer lock + handoff-state cleanup), Rev 3 silent accessory/add-on full-qty sweep (dashboard, invisible). **Darren's live re-test is the completion gate.** Prior: check-off initial ship (`ec2fe6e`/`87c75f2`, mig 096); warehouse IN TRANSIT writer (`eaedfb2`/`ff5c8eb`, 2026-06-09) |
| Latest merge | `8869246` (driver): inline `ItemCheckoffPanel` in the manifest slot + single gated bottom CTA; `truck_is_own` inspection-lock fix; `isTransferredAway`/`canComplete` ex-primary-vs-co-driver discrimination; `/api/complete-stop` clears handoff state at warehouse_return. `8c8fbcf` (dashboard): `SYNC_RENTAL_BODY` + write-back accessory/add-on sweep. Prior: `ec2fe6e` check-off initial ship |
...
Built 2026-06-09, build green, **not yet pushed** (pending two-device smoke test + chat-Claude dashboard/Notion mig-093 reconciliation). Driver-to-driver mid-route transfer of ownership. Spec: Notion MBC Part 3 (`3680aa6451b88160970dfa90027427e0`).
