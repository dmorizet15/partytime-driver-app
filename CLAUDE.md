...
| Item | Status |
|---|---|
| Active feature | **Warehouse IN TRANSIT writer** — `routes.actual_departure_at` stamped on route start (driver `eaedfb2` + dashboard `ff5c8eb`, **pushed to `main` both repos** 2026-06-09, both builds green). Prior: **Phase 2B — Route Handoff** (already on `origin/main`; CLAUDE.md previously said "NOT yet pushed" but `origin/main..HEAD` was empty — that note was stale); wall→ladder `dependency_map` data patch (`eac74f0`, 2026-06-06) |
| Latest merge | `eaedfb2` (driver) / `ff5c8eb` (dashboard): warehouse IN TRANSIT writer — `POST /api/routes/[routeId]/depart` stamps `routes.actual_departure_at`; dashboard `deriveStage`/`deriveColState` read it. Prior: Phase 2B Route Handoff (`3c3423e` etc., on `main`); wall→ladder data patch (`eac74f0`, 2026-06-06) |
