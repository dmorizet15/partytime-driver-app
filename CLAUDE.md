...
| Item | Status |
|---|---|
| Active feature | **Phase 2B — Route Handoff** built on `main` (2026-06-09), `npx next build` green, **NOT yet pushed** (held for two-device smoke test + chat-Claude mig-093 reconciliation — see Route Handoff section + `tasks/todo.md`). Prior: wall→ladder `dependency_map` data patch (`eac74f0`, 2026-06-06); Warehouse notes surfacing (`de05529`, 2026-06-03); AVA Phase 2 Session 3 (`48d5487`, 2026-06-02) |
| Latest merge | Direct to `main`: wall→ladder `dependency_map` data patch — keyword `sidewall`→`wall`, threshold 5→1, required_qty 2→1; recorded in `supabase/data-patches/wall-ladder-threshold-fix.sql` (`eac74f0`, 2026-06-06). Prior: warehouse notes surfacing (`de05529`, 2026-06-03); AVA SOP knowledge + role scoping (`48d5487`, 2026-06-02) |
| Latest migrations | 021 `dependency_map_tent_tools` + 020 `sop_entries_rls` + 019 `sop_entries`; local = 21 files. **Dashboard mig 093** (`routes.active_driver_id` + `transfer_pending_to`) was applied to the shared DB FROM this repo on 2026-06-09 (the prompt's "mig 092" claim was wrong — columns didn't exist; chat-Claude must mirror into the dashboard repo + fix the Notion ledger). **Non-migration DB data changes are recorded under `supabase/data-patches/`** (re-runnable `.sql`, applied via `supabase db query --linked --file`), NOT as numbered migrations. **Four data-patch files now exist** (incl. `2026-06-09_route-handoff-093-columns.sql`, a DDL schema patch) — all must be re-applied after a fresh DB rebuild |
