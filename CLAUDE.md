...
- **Active feature:** Fleet Maintenance Module (driver app) — shipped 2026-05-22 (`46ba851`), three UI fixes the same evening. Five screens (Fleet Overview, Asset Detail, Work Order Detail, Log Service Entry) + a role-gated Tools Hub card + a home-screen alert card. Reads/writes the dashboard-owned fleet tables (migrations 062–068) directly via the RLS-gated supabase client — no new API routes, no new migrations. See "Fleet Maintenance Module — Driver App" section below.
