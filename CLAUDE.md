...
- **Latest shipment:** **AVA Phase 2 — Session 1 (Weather Alerts + SOP Foundation), 2026-05-30 — merged to `main`/production via `git merge --no-ff feature/ava-phase2` (merge commit `0176699`); branch deleted (local + remote).** Migrations 018 (`dispatch_stops.delivery_lat/lng` geocache) + 019 (`sop_entries`). Weather: `geocodeAddress` (Nominatim, cache-first, server-side write-back), `getWindAtTime` (reuses the existing Tomorrow.io+NWS subsystem; returns **`max(sustainedMph, gustMph)`** — gust-inclusive — for the arrival hour, UTC-bucket matched), `POST /api/ava/route-weather` (server-side per-stop enrichment), `useRouteWeather` hook → `hasWeatherFlag` (wind-aware brief copy) + red `WIND {mph}` pills on Home stop cards (≥20 mph). SOP: `POST /api/sop/sync` mirrors the Notion SOP Library *page* (summary table + per-SOP child pages) into `sop_entries` — token-gated, **inert until `NOTION_API_KEY` is set** (returns 501). Plus: "Ask Ava about today" **placeholder** button (UI-only, coming-soon toast — real Haiku conversation sheet deferred), a Home stop-count fix (depot excluded from the driver-visible hero + section totals via `customerStopCount`; full `totalStopCount` stays on completion/empty/section gates), and a voice-first rewrite of the morning-brief copy. See "AVA Phase 2 (Driver App)" below + `docs/CHANGELOG.md`. **Smoke-test on production + set `NOTION_API_KEY`.**
- **Prior shipment:** **Fleet Maintenance driver app — Session 3, 2026-05-30 — commit `3bda5d7`, direct to `main`.** UI-only build of the four locked Fleet Maintenance screens (no migrations, no API routes — all via the existing RLS-gated supabase client). Screen 1 (Overview) → Trucks / Equipment / My Log pill tabs; truck cards gained current mileage + Reg/Insp/Ins compliance badges. Screen 2 (Asset Detail) → History / PM Schedule / Parts pill tabs, persistent open-WO block + persistent Log-service CTA. Screen 3 (Log Service) → mileage/hours prefill from current reading. Screen 4 (My Log, new) → current user's service records across all assets. Shared `PillTabs` + `ComplianceBadges` added; `PartCard` extracted from `WorkOrderDetailScreen`. Build green; pushed. **Smoke fixes follow-up (`0a1cb72`):** Bug 1 — My Log infinite spinner (a self-cancelling `useEffect`, not RLS; now loads keyed on `user.id` — see `tasks/lessons.md`); Bug 2 — Reg/Insp/Ins badges added to the Asset Detail header too (spec only locked them on the Overview list). Steps 1/3/4/5/6 passed round 1; **re-test My Log + Asset Detail badges on production.** See "Fleet Maintenance Module — Driver App → Session 3" below and `docs/CHANGELOG.md`.
- **Active feature:** **AVA Phase 2 — Session 1 is LIVE on `main`/production as of 2026-05-30** (merge `0176699`; branch `feature/ava-phase2` deleted). Next AVA work is **Phase 2 Session 2** on a NEW `feature/ava-phase2-sN` branch — top candidate: the real **Haiku-backed AVA conversation sheet** behind the "Ask Ava about today" placeholder + the AvaChip drawer (needs an Anthropic key + a server route + lifting the chip drawer into a shared `open/onClose/seedContext` component). Phase 2 spec source: the in-session step plan (this conversation) + Phase 1 master spec Notion `3550aa6451b881f19285e369387b75b6`. Phase 1 history below for context:
- **Prior feature:** **AVA Phase 1 was merged LIVE on `main` 2026-05-28** — merged via `git merge --no-ff feature/ava-phase1` (merge commit `37f83a9`) and pushed to production. All 9 original components shipped (profile preference columns, ava_conversations table, ava_stop_notes table, header chip, morning brief card, dependency-map + checklist, AVA Remembers UI, ElevenLabs TTS, voice/text toggle) PLUS the dispatcher-notes + stop-notes surface added on the branch (the "10 components" in the merge message). The `feature/ava-phase1` branch has been **deleted** (local + remote) — all history is in `main`. **Next AVA work goes on a NEW branch** (`feature/ava-phase2` or similar), not directly on `main` and not on the deleted branch. Phase 1 spec: Notion `3550aa6451b881f19285e369387b75b6`.
...
- **Latest migration:** **`20260530019_sop_entries`** (the `sop_entries` table + department index, AVA Phase 2 Session 1). Driver-app local `supabase/migrations/` is now **19 files** — Phase 2 added 018 `dispatch_stops_geocache` (`delivery_lat/lng`) + 019 `sop_entries`. Both applied via `supabase db query --linked --file` then `migration repair --status applied <version>` (the two-repo history block precludes `db push` — `migration list` shows ALL driver-app local migrations as local-only against a remote carrying the dashboard's migrations). Prior: 013/014/015 (Phase 1 Session 1), 016 `dependency_map`, 017 storage bucket. The "migration count" still tracks the remote table; dashboard owns most of the count, but the driver-app rows are in there too courtesy of the `migration repair` step.
- **Branch strategy:** AVA Phase 2 Session 1 is merged and its branch (`feature/ava-phase2`) deleted (see "Active feature" above). Going forward: unrelated work commits directly to `main`; the next AVA session gets its own `feature/ava-phase2-sN` branch and is not merged to `main` until Darren gives the go-ahead. Pattern confirmed this session: investigate-first → build per step → `npx next build` green → commit (one per deliverable) → push the branch → merge `--no-ff` only on Darren's explicit go.
...
- **Screen 4 — My Log** (new): the third Overview pill tab, **not a separate route**. Lists the signed-in user's `service_records` across all assets, newest first, via `fetchMyServiceLog(userId)` (filters `performed_by_user_id`). **Loads on `user.id` resolve (effect dep = `[user?.id]` only), NOT on tab-open** — the original tab-gated/loading-gated effect cancelled its own in-flight request and spun forever (fixed in `0a1cb72`; see `tasks/lessons.md`). Uses `<ServiceLogEntry assetName=…/>` (new optional asset line).

**Smoke fixes (2026-05-30, commit `0a1cb72`).** Bug 1 — My Log spinner race (above). Bug 2 — compliance badges (`<ComplianceBadges/>`) now also render in the **Asset Detail header** (trucks) next to the health pill; `fetchAssetInfo` computes `compliance` via `complianceStatus()` and `FleetAssetInfo` carries it (equipment → null). The spec only locked the badges on the Overview list cards; Darren asked for them on the detail screen too since that's where he acts on a compliance issue.
...
## AVA Phase 2 (Driver App)

Phase 2 builds on the merged Phase 1. **Session 1 (2026-05-30) — Weather Alerts + SOP Foundation — is LIVE on `main`/production** (merge `0176699`; branch `feature/ava-phase2` deleted). Spec source: the in-session step plan. Per-session detail in `docs/CHANGELOG.md`.

### Session 1 — Weather Alerts + SOP Foundation (2026-05-30, merge `0176699`)

**Migrations (applied via `db query --linked --file` + `migration repair`, never `db push`).**
- `20260530018_dispatch_stops_geocache.sql` — `dispatch_stops.delivery_lat/lng FLOAT` (nullable). Cached Nominatim coordinates so each address geocodes once.
- `20260530019_sop_entries.sql` — `sop_entries` (id, `sop_number` UNIQUE, title, content NOT NULL, department, version, effective_date, notion_page_id, last_synced_at, created_at, updated_at) + `sop_entries_department_idx`.

**Weather alert pipeline (all server-side except the hook + pill).**
- `src/lib/geo/geocodeAddress.ts` — `geocodeAddress(address, stopId?)`. Cache-first (reads `dispatch_stops.delivery_lat/lng` when `stopId` given), else Nominatim (`User-Agent: PartyTimeRentals-DriverApp/1.0`, 1 req/sec — `GEOCODE_RATE_LIMIT_MS` for batch callers), best-effort write-back. Returns null on any failure (graceful degrade). The DB branch uses the browser client, so server callers pass NO `stopId` (pure geocode) and do their own admin-client cache read/write — see the route.
- `src/lib/weather/weather-service.ts` → **`getWindAtTime(lat, lng, isoTime)`** added to the EXISTING Tomorrow.io+NWS subsystem (no Open-Meteo, no new file). Returns **`max(sustainedMph, gustMph)`** for the matching hour bucket — gust-inclusive, because gusts drive tent-stake risk (a 10 mph sustained / 21 mph gust hour must alert). `windHourly` is already imperial (adapter requests `units=imperial`) so NO km/h conversion. `windHourly[].time` is UTC — **callers MUST pass a UTC ISO time**; `windHourly` covers ~24h; null on miss/failure.
- `src/app/api/ava/route-weather/route.ts` — `POST { stopIds: [...] }`. Reads `dispatch_stops` authoritatively by id (admin client): address + cached coords + `calculated_eta` (the dispatcher cascade ETA — the stop's scheduled-arrival field; **nullable**). Per stop: cache-first geocode → admin write-back → `getWindAtTime(lat, lng, new Date(calculated_eta).toISOString())` (UTC-normalized so the bucket lines up with Tomorrow.io's UTC times). `weatherAlert = windMph >= 20`. Returns `{ stops: [{stopId, weatherAlert, windMph}], hasWeatherFlag }`. Auth-gated (session getUser); per-stop failures swallowed; 1 req/sec only on actual Nominatim calls.
- `src/hooks/ava/useRouteWeather.ts` — POSTs today's customer stop ids (depot excluded), returns `stop_id → {weatherAlert, windMph}` map + `hasWeatherFlag`; fails open. Keyed on the stable id-set string so it refetches only when the set changes.
- **Wiring:** `DayRouteSelectorScreen` calls the hook, passes `hasWeatherFlag` into `AvaMorningCard` (→ `MorningSummary` → `getMorningMessage`'s existing wind copy) and uses the per-stop map for a red **`WIND {mph}`** pill (#DC2626 / white, payment-pill sizing, rounded mph) on both the COD card and standard stop-row layouts.

**SOP foundation.** `src/app/api/sop/sync/route.ts` — `POST /api/sop/sync`. The Notion SOP Library (`3590aa6451b8816aa156c77f605facfe`) is a **page**, not a database: a summary TABLE block holds metadata (SOP # / Title / Version / Effective Date / Department) and one CHILD PAGE per SOP holds the procedure text. The route reads page children via raw Notion REST (`fetch`, no `@notionhq/client` dep), parses the table for metadata, enumerates child pages for `sop_number`/`title`/`content`/`notion_page_id` (bounded recursive body-text extraction; `content` falls back to title since it's NOT NULL), merges, and **upserts by `sop_number`** (admin client). Auth: `x-sop-sync-secret` header (cron) OR authenticated session. **Inert until `NOTION_API_KEY` is set** → 501, never touches the DB. Session 1 scope is table + sync endpoint ONLY — no search UI.
- **Env (driver-app Vercel, server-only):** `NOTION_API_KEY` (required for sync; share the integration into the SOP Library page) + optional `SOP_SYNC_SECRET` (cron header). Documented in `.env.local.example`.

**"Ask Ava about today" — PLACEHOLDER only.** `src/components/ava/AskAvaButton.tsx` — gold (+) circle + label on Home (pre-pre-trip, between the stop list and the Inspect CTA). Tap → coming-soon toast. The real Haiku-backed conversation sheet is **deferred** — note: `AvaChip`'s drawer is still a self-contained placeholder (private `open` state, no external open hook, no context prop, no Claude call). Building the real Step 6 means a shared `open/onClose/seedContext` sheet + an Anthropic key + a server route (`claude-haiku-4-5-20251001`).

**Home stop-count fix.** Hero "N stops scheduled" used `dayStops.length` (depot included) while the type breakdown excluded depot → "3 stops … 1 delivery · 1 pickup" mismatch. Added `customerStopCount` (depot-excluded) for the two **driver-visible** totals (hero sub-copy + "The day, in N" section header); `totalStopCount` (full) stays on the route-complete gate, `isEmpty`, and section guards (depot must count toward "all done").

**Morning-brief copy.** Voice-first rewrite of all `getMorningMessage.ts` template strings for ElevenLabs Creator prosody (em-dash breaths, short sentences, contractions, no comma+and run-ons). Copy only — branch structure/conditions/`windTail`+`weatherTail` mechanics unchanged. `MorningSummary.stopCount` is already depot-excluded (card feeds `customerStops.length`).

**NEXT smoke test (production — Vercel auto-deploys `0176699`):**
1. Set `NOTION_API_KEY` on driver-app Vercel + share the integration into the SOP Library page → `POST /api/sop/sync` (with a session or the secret header) returns `{ synced: N, errors: [] }`; `sop_entries` populates (10 SOPs).
2. Route with a stop forecast ≥20 mph (sustained OR gust) at its `calculated_eta` → Home shows a red `WIND {n}` pill on that stop card AND the morning brief includes the wind line. Gust-only case (e.g. 10 sustained / 21 gust) must alert.
3. Route ending at the depot → hero reads "N stops scheduled" matching the "1 delivery · 1 pickup" breakdown (depot NOT counted); "The day, in N" matches too.
4. "Ask Ava about today" gold (+) button renders between the stop list and the Inspect CTA → tap shows the coming-soon toast.
5. Voice brief reads naturally (the long wind-advisory variant is the one to ear-check against the 0.6s sentence pause).
