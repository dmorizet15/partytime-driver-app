...
| Item | Status |
|---|---|
| Active feature | None in flight — **wall→ladder `dependency_map` data patch** pushed direct to `main` (`eac74f0`, 2026-06-06). Prior: **Warehouse notes surfacing** (route- + stop-level, `de05529`, 2026-06-03); AVA Phase 2 **Session 3** (SOPs in `/api/ava/ask` + role scoping, `48d5487`, 2026-06-02). Next AVA work → new `feature/ava-phase2-sN` branch |
...
| Latest migrations | 021 `dependency_map_tent_tools` + 020 `sop_entries_rls` + 019 `sop_entries`; local = 21 files. **Non-migration DB data changes are recorded under `supabase/data-patches/`** (re-runnable `.sql`, applied via `supabase db query --linked --file`), NOT as numbered migrations. **Three data-patch files now exist** — all must be re-applied after a fresh DB rebuild |
