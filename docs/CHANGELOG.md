...
- **HAL (device-neutral by design):** `BridgeEventMap` gains `'trigger-event'` (flows through `windowRfidBridge` + `ScriptedBridge` automatically — both are generic over the event map). `RfidScanner` gains `onTrigger(cb)` + `capabilities.hardwareTrigger`. Contract point: the trigger subscription is deliberately ALIVE regardless of `initialize()`/`release()` — a press is what CAUSES the radio to come up, so it must fire while powered down. `Xr2Scanner` implements it over the bridge (lazy subscription, defensive consecutive-same-edge drop against key-repeat leaks); `MockScanner` gains `emitTrigger(pressed)` so tests can drive it; `C72Scanner` stays an honest stub (`hardwareTrigger: false`, `onTrigger` throws).
- **`useHardwareTrigger` (provider/):** subscribes once per scanner (feature-gated on the capability) and routes edges into a latest-value handlers ref that each screen assigns to its OWN `scanStart`/`scanEnd` — the exact handlers the on-screen HOLD TO SCAN button calls. So first-tag-wins + radio-drop-mid-hold (individual), accumulate-then-commit (mass), and the pickup/status armed-status gate behave identically for hardware and on-screen holds, by construction rather than by parallel implementation. Wired into Delivery Checkout, Pickup Return, and Touch Scan.
- **Tests:** 6 new — HAL edge forwarding pre-initialize, repeat-drop, unsubscribe/release survival, C72 stub, screen-level hardware-hold individual (first tag captures, radio drops WHILE held) + mass (accumulates until release) + the pickup no-status gate (a hardware press with nothing armed never starts the radio). Two test iterations were instructive: the radio-drop assertion had to move behind `waitFor` (capture resolves against the replica asynchronously), and the mass re-scan needed different EPCs (the 500ms window dedupe from the individual pull was still live — the test initially fought the exact behavior the model requires). Suite 78 (77 + 1 gated live); tsc + `next build` green.
- **On-device verification attempted, honestly BLOCKED on the device screen PIN.** Rig restored (dev server + `adb reverse` + CDP attach): the XR2 is USB-attached, the Session-14 wrapper build is installed, and — notably — a driver session was ALREADY logged into the WebView (sitting on a live stop-detail page), clearing Session 14's "need creds on the device" blocker. But the screen is PIN-locked with no PIN on record (not guessed): the raw trigger key only reaches the activity with window focus, and the locked-screen renderer throttles IndexedDB to ~2.5 min/op (a probed `indexedDB.open` took 158s) so the module runtime never leaves "Starting scanner…". Framework-side sanity DID pass while locked: `input keyevent 523` produced the full `BaseKeyMapManager` + `TRIGGER_EVENT` broadcast chain in logcat (TriggerProbe received it). Full diagnostic ladder recorded in `tasks/lessons.md`; the 10-minute post-unlock checklist is in `tasks/todo.md`.
- **New ASSUMPTIONS.md entry:** hardware trigger × armed-Barcode interaction (the framework may drive the imager on the same physical hold the web layer treats as an RFID hold).
- Guardrails intact: zero writes anywhere, TapGoods dry-run locked, `EASY_RFID_ALLOW_PRODUCTION` unset, driver-app `main` untouched, no force-pushes.

**Same-day epilogue — Darren returned, unlocked the XR2, and the physical verification PASSED (`c0db9ef`).** Getting there surfaced two real problems device contact alone could find: (1) the long-running dev server had wedged (404s on its own `main-app.js`/layout chunks → the WebView rendered SSR HTML with no client runtime — "Starting scanner…" forever; fixed by a clean `rm -rf .next` restart + fresh app process, since the stale renderer's saturated connection pool survived the server restart); (2) a genuine browser-only module bug — `HttpTagBackend`/`ezrfidClient` stored the bare global `fetch` and invoked it as `this.fetchImpl(...)`, which passes the class instance as receiver → `TypeError: Illegal invocation` in the WebView, misreported by the catch as a network failure ("Item list refresh failed"). Node's fetch ignores the receiver, so the entire suite (including Session 12's live harness) could never catch it. Both sites now wrap the global; a regression test installs a receiver-strict fake fetch; the Touch Scan sync catch now logs the underlying error (the silent swallow hid this for half an hour). Then, with the fix hot-reloaded and 12,991 records seeded on-device: **Darren's physical trigger holds drove the corrected scan model end to end** — individual first-tag-wins (tag `202602070015FFFF` captured, radio dropped mid-hold, Clear offered), mass accumulate (13 distinct tags in one hold, real Item Master names/statuses resolving instantly from the on-device replica, honest "— unknown tag —" rows for ambient EPCs), `/dev/ttyHSL0` open during holds and closed on release. Operational note: each press pays the ~2s radio init (Session 1 lesson observed driver-side) — a sub-second squeeze reads nothing.
...
- **Guard alignment (blocking step, confirmed).** The card's inflatable "No earlier than" floor is `effectiveWindow(pickupStop).startsAt` — the IDENTICAL value the early-pickup standby/countdown guard enforces (`stopConstraints.ts`; StopDetailScreen `pickupOpensAt = effectiveWindow(stop).startsAt`). Narrowed `effectiveWindow`'s param to a structural `WindowResolvable` interface so the card's lighter pickup rows reuse the one resolver (Stop still satisfies it → all existing callers unaffected). Promise and block can never disagree.
- **Pure derivation `src/lib/pickupAnswer/{derive,types,classify,format}.ts`.** Input: delivery stop + reservation pickups → `{ kind, status, floorTime, etaTime, day, window, trips[] }`. Reservation-scoped, dedup phantom rows by `scheduled_date` (18 live same-date multi-pickup reservations), grouped by `reservation_id` (not `linked_stop_id`, which can chain pickup→pickup). Inflatable floor = `COALESCE(dispatcher_time_override, pickup_window_start)`; live ETA = `calculated_eta`; never `scheduled_date` (drifts) for the promise. Confidence-gated split labels (only two distinct dates + cleanly-separable real inflatable + real tent, else neutral "First/Second trip"). America/New_York, DST-safe (`Intl`). **Smoke 45/45 PASS.**
- **Classifier `classify.ts`.** Reuses `isInflatableCategory` VERBATIM against BOTH category and name (category `INFLATABLES` leaks into GAMES/Misc/blank; keyword-less names like "MECHANICAL BULL" are caught by category) + anchor-accessory exclusions; tent = pure mirror of `isTentItem` (its module imports the supabase client → can't be pulled into a pure module).
- **Endpoint `GET /api/stops/pickup-answer?stop_id=`** (service-role + cookie; the reservation's pickups are on other routes → client RLS can't read them). Derives server-side, returns `{ ok, answer }`; the "no pickup scheduled" copy shows ONLY on a confirmed `ok:true, kind:'none'` (a transient failure → `ok:false` → nothing).
- **Card `src/components/pickup/PickupAnswerCard.tsx`** (delivery-only, under the customer card, above the manifest). Collapsed glanceable (clock/calendar glyph) → tap-expand; two-line model (bold promise + pulsing live-ETA line); "Tell the customer" say-line; per-trip rows for splits; reuses the guard's `formatCountdown` for an imminent same-day floor. No reschedule action.
- **Live-validated:** delivery `5ab4e402` (EMILY CHAMBERS, 07-07) → card shows "No earlier than 12:30 PM" + "Currently expected ~3:30 PM", floorTime === pickup_window_start === guard value. `next build` green (38 pages). VERSION 2.4.0 → **2.5.0** (new feature → MINOR). **Pending: Darren device smoke** — targets in `tasks/todo.md`.
