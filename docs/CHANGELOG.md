...
## 2026-07-06 — Pickup Answer (Driver-Facing) — gold card on the delivery stop (branch `feat/pickup-answer`, NOT pushed; VERSION 2.5.0; no migration)

Two-session build (investigation + pure derivation, then UI + wiring) on the MBC Part 3 spec (locked 2026-07-05). A read-only gold card on the DELIVERY stop that instantly answers "when are you picking up?" from the reservation's pickup stop(s). Left on a feature branch for Darren's device smoke — NOT pushed to `main`.
