...
## Verify the interaction model against the legacy tool BEFORE building a replacement workflow — data contracts surviving review doesn't mean the workflow shape did.

**Why:** 2026-07-13 the whole RFID scan model had to be rebuilt: it shipped scan-first/flag-after with toggle triggers and an invented safe default ('Needs to be Inspected'), when the legacy Easy RFID Pro model — and the crews' muscle memory — is status-FIRST (arm, then scan; press-and-hold trigger; individual = one tag per pull). The API contract work (upsert shape, wrapped 401s, status vocabulary) was all correct; the *sequence of driver actions* was wrong, and every screen + the flow engine + the tests had to change. An invented default is the same trap as the equipment-return "missing row means nothing was retrieved" bug on main: writing a value nobody chose creates false data at fleet scale.

**How to apply:** For any workflow that replaces an existing tool, capture the legacy interaction sequence (what is chosen before/after the physical action, what a trigger press does, what un-acted-on items get) as spec-level facts with the same rigor as API payloads — screenshots or a narrated walkthrough of the legacy app, logged in the feature spec. Never invent a default status for unhandled items: no decision must mean NO write. Two implementation traps from the rework: (1) an event subscription made inside `start()` stacks under press-and-hold start/stop cycles — subscribe once per session and regression-test hold cycles; (2) with a shared fake-indexeddb across a test file, re-seeding preserves pending local overlays by design — point each test's writes at fixtures no other test asserts on.
