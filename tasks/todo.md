...
- [x] **ON-DEVICE VERIFICATION — PASSED same day** (Darren returned, unlocked the XR2, worked the trigger physically): Individual hold → first tag captured (`202602070015FFFF`) + radio dropped mid-hold + Clear offered; Mass hold → 13 distinct tags accumulated with instant replica resolution (12,991 records seeded on-device via Sync item list); trigger release closes `/dev/ttyHSL0`. Found + fixed a device-only bug in the process (`c0db9ef`, unbound global fetch → Illegal invocation in the WebView — regression-tested). Note for future holds: the press pays the ~2s radio init, so sub-second squeezes read nothing.
- [ ] Pickup Return armed-status gate on-device (jsdom-tested; same `scanStart` gate the verified trigger path calls — low risk, verify opportunistically at the next device session).
