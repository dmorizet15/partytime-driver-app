...
- [ ] **ON-DEVICE VERIFICATION — BLOCKED ON THE DEVICE PIN (THE gate).** The XR2 is USB-attached with the Session-14 wrapper installed AND a driver session already logged into the WebView (the Session-14 "need creds on device" blocker is CLEARED — the WebView was sitting on a live stop detail page). But the device screen is PIN-locked ("Enter your PIN", no PIN on record — not guessed): the raw trigger key only reaches the activity when it has window focus, and the locked-screen WebView throttles IndexedDB to ~2.5 min/op so the module runtime can't even start (see lessons). **Once someone unlocks the device:** `npm run dev` + `adb reverse tcp:3000 tcp:3000`, open the wrapper → Tools → Touch Scan; physically hold the side trigger: Individual = first tag captures + radio drops mid-hold + Clear re-pulls; Mass = accumulate while held, stop on release; Pickup Return with NO status armed = trigger press does nothing. `adb shell input keyevent --longpress 523` approximates a short hold if hands aren't free.
- [ ] Hardware trigger × armed-Barcode interaction — see the new ASSUMPTIONS.md entry (framework may drive the imager on the same physical hold).
- [ ] SECONDARY (untouched, per session brief — primary not device-verified): barcode symbology-id → display-name mapping ("9" = UPC etc.), or show the raw id honestly.
