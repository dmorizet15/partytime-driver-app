...
- [ ] **Smoke-test Party Kong on production** — coverage in `CLAUDE.md` → "Party Kong" NEXT block. Loops: hub tile flipped to PLAY, 4-level progression, ladder climb hints, chair stack dollies introduced L2, win condition at right side of P4, game-over + leaderboard submit + cross-device realtime fan-out.
