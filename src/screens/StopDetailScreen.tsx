...
export default function StopDetailScreen({ routeId, stopId }: StopDetailScreenProps) {
  ...
  // Auto-clear navigation inline message after 5 seconds
  useEffect(() => {
  ...
  async function handleNavigate() {
    ...
    try {
      ...
      if (!result.success) {
        logEvent('NAVIGATION_FAILED', routeId, stopId, stop.order_id, {
  ...
  // ── RFID — three-state logging: attempt → success (dispatch) or failed ───
  function handleOpenRfid() {
    if (!stop) return

    // Always log that the driver tapped the button
    logEvent('RFID_APP_OPENED_ATTEMPT', routeId, stopId, stop.order_id)

    const result = externalAppService.launchRfidApp()

    if (result.success) {
      // Intent was dispatched to Android PackageManager without a JS exception.
      // Note: this confirms dispatch only — not that the app visibly opened.
      logEvent('RFID_APP_OPEN_SUCCESS', routeId, stopId, stop.order_id, {
        url: result.url,
      })
      // DEBUG: show intent URL on screen so the driver can confirm dispatch
      // (auto-clears in 5 s; remove once RFID launch is confirmed working on device)
      setRfidMessage(`[DEBUG] Intent dispatched: ${result.url ?? 'unknown'}`)
    } else {
      // Either blocked (non-Android, attempted:false) or a JS exception occurred.
      logEvent('RFID_APP_OPEN_FAILED', routeId, stopId, stop.order_id, {
        attempted: result.attempted,
        message: result.message,
      })
      // Surface inline message — do NOT alert() or navigate away
    ...
    <div className="screen">
      ...
      <div className="flex-1 overflow-y-auto pb-8">
        ...
        <div className="px-4 pt-4">
          ...
          {/* Inline message — shown on browser, auto-clears after 5s */}
          {navMessage && (
            <p className="text-[11px] text-gray-500 text-center mt-1.5 mb-2 px-3 py-2
